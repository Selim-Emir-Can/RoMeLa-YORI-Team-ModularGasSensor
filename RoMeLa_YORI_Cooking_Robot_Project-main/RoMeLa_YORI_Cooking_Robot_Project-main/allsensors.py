...
########################################### ECXECUTION OF PROCESS #################################################################

arr_oz = np.zeros(1)
arr_voc = np.zeros(1)
sgp = SGP40()
while((np.size(arr_oz) < 10) and (np.size(arr_voc) < 20)): #NEED TO CHANGE THIS CONDITION LATER
  ''' Smooth data collection the collection range is 1-100 '''
  arr_oz = np.append(arr_oz, ozone.get_ozone_data(COLLECT_NUMBER))
  print("Raw Gas: ", sgp.raw())
  arr_voc  = np.append(arr_voc , sgp.measureRaw(25, 50))
  print("measureRaw Gas: %d" %arr_voc [np.size(arr)-1])
  print("Ozone concentration is %d PPB."%arr_oz[np.size(arr) - 1])
  time.sleep(1)
  

arr_oz = np.delete(arr_oz, 0)
print(arr_oz)
arr_voc  = np.delete(arr_voc, 0)
print(arr_voc)
...
def createCSVfile():
    ...
    writer = csv.writer(f)
    writer.writerow(arr_oz)
    writer.writerow(arr_voc)
    f.close()
